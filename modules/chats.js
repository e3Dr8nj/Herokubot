...
,description:[[" "," Сделать закрытый всем маня мирок в войсе. (*ПО УМОЛЧАНИЮ РАЗБЛОКИРОВАН*)(Уже находящимся в войсе выдадутся права доступа с возможностью говорить)",'0']]
,help_type:'base'
,run:async(client,message,args)=>{try{
        let obj=await exports.getProps(client,message,args);  if(!obj.any) return obj.any_no();
...
,description:[[" "," Та же команда что и выше, но выкинет из войса всех кому прежде не был выдан персональный доступ( право в режиме слушателя/спикера).",'0']]
,help_type:'base'
,run:async(client,message,args)=>{try{
        let obj=await exports.getProps(client,message,args); if(!obj.any) return obj.any_no();
        let voice_chat = message.guild.channels.cache.get(exports.text_channels[message.channel.id].voice_channel.id); if(!voice_chat) return; console.log('2');
...
module.exports.commands.giveAccess={aliase:'спикер'
...
module.exports.commands.giveListen={aliase:'слушатель'
