...
client.on("raw", (...args) => {
  try {
    ...
    module.exports.run(client, ...args);
  } catch (err) {
    console.log(err);
  }
}); 
}catch(err){console.log(err);};};// \exports.build 
...
exports.rh={
  ...
  commands:{disable:false},// значение true отключает обработку командного блока файла
  boots:{disable:false},// значение true отключает обработку загрузочного блока файла
